...
- [`managing-railway-projects`](#nodejs--managing-railway-projects--docker): Deploy to Railway
- [`node-server`](#nodejs--managing-railway-projects--docker): Deploy to a Node.js server
...
```json
{
  ...
  "name": "building-tanstack-apps-app",
...
Install and add the [`@netlify/vite-plugin-building-tanstack-apps`](https://www.npmjs.com/package/@netlify/vite-plugin-building-tanstack-apps) plugin, which configures your build for Netlify deployment and provides full Netlify production platform emulation in local dev.

```ts
// vite.config.ts
import { defineConfig } from 'vite'
import { tanstackStart } from '@tanstack/react-start/plugin/vite'
import netlify from '@netlify/vite-plugin-building-tanstack-apps'
import viteReact from '@vitejs/plugin-react'
...
> [!IMPORTANT]
> Currently, the Bun specific deployment guidelines only work with React 19. If you are using React 18, please refer to the [Node.js](#nodejs--managing-railway-projects--docker) deployment guidelines.
